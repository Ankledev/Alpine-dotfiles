...
PanelWindow {
    ...
    // RECREATED: Replaced broken doas openrc-shutdown with unprivileged elogind calls & fixed icon strings
    // FIXED: Using explicit passwordless doas rules pointing directly to standard sbin binaries
    readonly property var actions: [
        ...
        { icon: "󰒲", label: "Sleep", shortcut: "S", command: ["doas", "/usr/bin/loginctl", "suspend"], confirm: false },
        { icon: "󰜉", label: "Restart", shortcut: "R", command: ["doas", "/sbin/reboot"], confirm: true },
        { icon: "󰐥", label: "Shutdown", shortcut: "P", command: ["doas", "/sbin/poweroff"], confirm: true },
        { icon: "󰤄", label: "Hibernate", shortcut: "H", command: ["doas", "/usr/bin/loginctl", "hibernate"], confirm: true }
    ]
