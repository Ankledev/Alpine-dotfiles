...
PanelWindow {
    ...
    // Alpine/elogind friendly actions (no doas/sudo required)
    readonly property var actions: [
        ...
        { icon: "󰒲", label: "Sleep", shortcut: "S", command: ["loginctl", "suspend"], confirm: false },
        { icon: "󰜉", label: "Restart", shortcut: "R", command: ["loginctl", "reboot"], confirm: true },
        { icon: "󰐥", label: "Shutdown", shortcut: "P", command: ["loginctl", "poweroff"], confirm: true },
        { icon: "󰤄", label: "Hibernate", shortcut: "H", command: ["loginctl", "hibernate"], confirm: true }
    ]
    ...
    Item {
        ...
        Canvas {
            ...
            opacity: 0.76
            onPaint: {
                // Changed const to var to satisfy QML parser
    ...
    function handleKey(event) {
        ...
        if (event.key === Qt.Key_Left) {
            ...
            var key = String(event.text || "").toUpperCase()
    ...
    function runAction(index) {
        ...
        actionRunner.exec(actions[index].command)
        closeAnimated()
    }
