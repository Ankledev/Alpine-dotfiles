...
Column {
    ...
    Timer {
        ...
        onTriggered: {
            root.stage++
            if (root.stage === 0) root.headerOffset = 0
            else if (root.stage === 1) root.bodyOffset = 0
            if (root.stage >= 1) stop()
        }
    }
    ...
    Item {
        ...
        Column {
            ...
            Text {
                width: parent.width
                text: root.errorMessage.length > 0 ? root.errorMessage : (!root.nmcliAvailable ? "wpa_cli not found" : (!root.wifiEnabled ? "WiFi is disabled" : "No networks in range"))
                color: root.m3OnSurface
                font.pixelSize: 16
                font.bold: true
            ...
            Row {
                ...
                PillButton {
                    theme: root.theme
                    text: !root.wifiEnabled ? "Enable WiFi" : "Refresh"
                    width: 128
        ...
        Column {
            ...
            Repeater {
                ...
                Rectangle {
                    ...
                    radius: root.theme.controlRadius
                    clip: true
                    color: headerArea.pressed ? root.theme.withAlpha(root.m3Primary, 0.10) : (headerArea.containsMouse || rowExpanded || rowSelected ? root.theme.withAlpha(root.m3Primary, 0.055) : "transparent")
                    ...
                    Column {
                            ...
                            Column {
                                ...
                                Rectangle {
                                    ...
                                    Rectangle {
                                        ...
                                        Behavior on width { SpringAnimation { duration: root.theme.reducedMotion ? 0 : 280; spring: 4.8; damping: 0.84; mass: 0.9; epsilon: 0.001 } }
                                    }
                                }
                            }
                        ...
                        Column {
                            ...
                            Rectangle {
                                ...
                                height: 38
                                radius: 14
                                color: rowRoot.autoconnect ? root.theme.withAlpha(root.m3Primary, 0.18) : root.theme.withAlpha(root.m3OnSurface, 0.055)
                                border.width: 0
                                ...
                                Rectangle {
                                    ...
                                    Row {
                                        ...
                                        BusyIndicator {
                                            width: 18
                                            height: 18
                                            visible: rowRoot.rowConnecting && root.connectingSsid === rowRoot.rowSsid
                                            running: rowRoot.rowConnecting && root.connectingSsid === rowRoot.rowSsid
                                        }
                                        Text {
                                            text: (rowRoot.rowConnecting && root.connectingSsid === rowRoot.rowSsid) ? "Connecting..." : "Connect"
                                            color: root.theme.foreground
                                            font.pixelSize: 14
                                            font.bold: true
                                        }
                                    ...
                                    MouseArea {
                                        ...
                                        enabled: !rowRoot.rowConnecting
                                        cursorShape: Qt.PointingHandCursor
                                        onClicked: root.connectNetwork(rowRoot.rowSsid, true, "", true)
                                    }
                                }
                                WifiActionButton {
                                    theme: root.theme
                            ...
                            Row {
                                ...
                                WifiActionButton {
                                    ...
                                    width: 70
                                    onClicked: {
                                        if (rowRoot.confirmAction === "disconnect") root.disconnectNetwork(rowRoot.rowSsid)
                                        else root.forgetNetwork(rowRoot.rowSsid)
                                    }
                                }
                                WifiActionButton {
                                    theme: root.theme
    ...
    Process {
        ...
        onExited: function(exitCode, exitStatus) {
            ...
            if (!root.nmcliAvailable) {
                root.loading = false
                root.errorMessage = "wpa_cli binary not found"
                networkModel.clear()
                return
            }
            root.errorMessage = ""
            deviceProbe.exec(["sh", "-c", "iwmq=$(ip link | grep -E 'wlan[0-9]|wlp'); echo \"$iwmq\" | awk -F': ' '{print $2}' | head -n 1"])
        }
    }
    ...
    Process {
        ...
        onExited: function(exitCode, exitStatus) {
            var dev = deviceOut.text.trim();
            if (dev.length > 0) root.wifiDeviceName = dev
            radioProbe.exec(["/sbin/wpa_cli", "-i", root.wifiDeviceName, "status"])
        }
    }

    Process {
        id: radioProbe
        stdout: StdioCollector { id: radioOut; waitForEnd: true }
        stderr: StdioCollector { id: radioErr; waitForEnd: true }
        onExited: function(exitCode, exitStatus) {
            if (exitCode !== 0) {
                root.loading = false
                root.wifiEnabled = false
                root.errorMessage = "wpa_supplicant not responding"
                networkModel.clear()
                return
            }
            var res = radioOut.text
            root.wifiEnabled = !res.includes("wpa_state=INTERFACE_DISABLED")
            if (!root.wifiEnabled) {
                root.loading = false
                networkModel.clear()
                return
            }
            savedProbe.exec(["/sbin/wpa_cli", "-i", root.wifiDeviceName, "list_networks"])
        }
    }

    Process {
        id: savedProbe
        stdout: StdioCollector { id: savedOut; waitForEnd: true }
        stderr: StdioCollector { id: savedErr; waitForEnd: true }
        onExited: function(exitCode, exitStatus) {
            root.parseSavedConnections(savedOut.text)
            listProbe.exec(["sh", "-c", "/sbin/wpa_cli -i " + root.wifiDeviceName + " scan > /dev/null && sleep 5 && /sbin/wpa_cli -i " + root.wifiDeviceName + " scan_results"])
        }
    }
    ...
    Process { id: wifiPowerProcess; onExited: function(exitCode, exitStatus) { root.refresh() } }
    Process { id: actionProcess; onExited: function(exitCode, exitStatus) { if (exitCode !== 0) { root.connectingSsid = ""; root.errorSsid = root.actionSsid; root.errorMessage = "Connection action failed" } else root.finishActionSuccess() } }
    Process { id: autoconnectProcess; onExited: function(exitCode, exitStatus) { root.finishActionSuccess() } }

    function refresh() {
        if (nmcliCheck.running || deviceProbe.running || radioProbe.running || savedProbe.running || listProbe.running) return
        loading = true
        errorSsid = ""
        errorMessage = ""
    ...
    function setWifi(enabled) {
        loading = true
        var op = enabled ? "ENABLE" : "DISABLE"
        wifiPowerProcess.exec(["/sbin/wpa_cli", "-i", root.wifiDeviceName, op])
    }
    ...
    function connectNetwork(ssid, useSavedConnection, password, autoconnect) {
        ...
        if (useSavedConnection) {
            var netId = savedConnectionNames[ssid] || "0"
            actionProcess.exec(["sh", "-c", "/sbin/wpa_cli -i " + root.wifiDeviceName + " select_network " + netId + " && /sbin/wpa_cli -i " + root.wifiDeviceName + " save_config"])
        } else {
            var script = "ID=$(/sbin/wpa_cli -i " + root.wifiDeviceName + " add_network | tail -n 1); " +
                         "/sbin/wpa_cli -i " + root.wifiDeviceName + " set_network $ID ssid '\"" + ssid + "\"' > /dev/null; " +
                         (password.length > 0 ? "/sbin/wpa_cli -i " + root.wifiDeviceName + " set_network $ID psk '\"" + password + "\"' > /dev/null; " : "/sbin/wpa_cli -i " + root.wifiDeviceName + " set_network $ID key_mgmt NONE > /dev/null; ") +
                         "/sbin/wpa_cli -i " + root.wifiDeviceName + " select_network $ID && /sbin/wpa_cli -i " + root.wifiDeviceName + " save_config"
            actionProcess.exec(["sh", "-c", script])
        }
    }
    ...
    function disconnectNetwork(ssid) {
        ...
        errorSsid = ""
        errorMessage = ""
        actionProcess.exec(["/sbin/wpa_cli", "-i", root.wifiDeviceName, "disconnect"])
    }

    function forgetNetwork(ssid) {
        action = "forget"
        actionSsid = ssid
        var netId = savedConnectionNames[ssid] || ""
        if (netId.length > 0) actionProcess.exec(["sh", "-c", "/sbin/wpa_cli -i " + root.wifiDeviceName + " remove_network " + netId + " && /sbin/wpa_cli -i " + root.wifiDeviceName + " save_config"])
        else finishActionSuccess()
    }
    ...
    function parseSavedConnections(text) {
        ...
        for (var i = 1; i < lines.length; i++) {
            ...
            if (cols.length >= 2) {
                ...
                if(name && !isNaN(id)) known[name] = id
            }
        }
    ...
    function parseNetworks(text) {
        ...
        // Skip the header line ("bssid / frequency...")
        for (var i = 1; i < lines.length; i++) {
            var line = lines[i].trim();
            if (line.length === 0) continue;
            
            // Use a regex to split by one or more tabs
            // This is safer than a simple \t split
            var cols = line.split(/\t+/);
            
            // If the SSID is missing (the 5th column), treat it as hidden/empty
            // We only need the first 4 columns to have data (BSSID, Freq, Level, Flags)
            if (cols.length < 4) continue;

            var ssid = (cols.length >= 5) ? cols[4].trim() : "Hidden Network";
            
            var dbm = parseInt(cols[2], 10) || -100;
            var signal = Math.max(0, Math.min(100, 2 * (dbm + 100)));
            var flags = cols[3] ? cols[3].trim() : "";
            var isSecure = flags.includes("WPA") || flags.includes("WEP");
            var isKnown = savedConnectionNames[ssid] !== undefined;

            // Only add if it's a valid network we want to see
            if (!bySsid[ssid] || signal > bySsid[ssid].strength) {
                bySsid[ssid] = { 
                    ssid: ssid, 
                    strength: signal, 
                    security: isSecure ? "WPA2" : "--", 
                    inUse: false, 
                    known: isKnown 
                };
            }
        ...
        // ... (Keep the rest of your p.onExited logic exactly as it is)
        var p = Qt.createQmlObject('import Quickshell.Io; Process { command: ["/sbin/wpa_cli", "-i", "' + root.wifiDeviceName + '", "status"]; stdout: StdioCollector { waitForEnd: true } }', root);
        p.onExited = function(code) {
            var outText = p.stdout.text();
            var match = outText.match(/ssid=(.*)/);
            if (match && match[1]) {
                var activeName = match[1].trim();
                if (bySsid[activeName]) bySsid[activeName].inUse = true;
            }
            
            var parsed = [];
            for (var key in bySsid) {
            ...
            root.loading = false; // Ensure loading stops
        };
        p.exec(p.command);
    }
        var p = Qt.createQmlObject('import Quickshell.Io; Process { command: ["/sbin/wpa_cli", "-i", "' + root.wifiDeviceName + '", "status"]; stdout: StdioCollector { waitForEnd: true } }', root)
        p.onExited = function(code) {
            ...
            var parsed = []
            for (var key in bySsid) parsed.push(bySsid[key])
            parsed.sort(function(a, b) {
                if (a.inUse !== b.inUse) return a.inUse ? -1 : 1
                return b.strength - a.strength
            })
            networkModel.clear()
            for (var j = 0; j < parsed.length; j++) networkModel.append(parsed[j])
    ...
    function activateSelectedNetwork() {
        if (selectedIndex < 0 || selectedIndex >= networkModel.count) return
        var item = networkModel.get(selectedIndex)
        if (!item) return
        if (item.inUse) disconnectNetwork(item.ssid)
        else if (item.known) connectNetwork(item.ssid, true, "", true)
        else toggleExpanded(item.ssid)
    }

    function signalIconSource(value) {
        var pct = Math.max(0, Math.min(100, Math.round(Number(value))))
        if (pct <= 25) return wifiIconDir + "wifi_item_1_bar.svg"
        if (pct <= 50) return wifiIconDir + "wifi_item_2_bar.svg"
        if (pct <= 75) return wifiIconDir + "wifi_item_3_bar.svg"
